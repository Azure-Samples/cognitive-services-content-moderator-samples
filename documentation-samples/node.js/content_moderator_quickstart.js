...
const IMAGE_MODERATION_URLS = ['https://moderatorsampleimages.blob.core.windows.net/samples/sample2.jpg',
'https://moderatorsampleimages.blob.core.windows.net/samples/sample5.png']

// Text moderation variables
const TEXT_MODERATION_FILE = 'text_file.txt'
const outputFile = 'text_modertion_output.txt';
...
async function main() {
    ...
    let image_racy_result = await client.imageModeration.evaluateUrlInput("application/json", imageBodyModel)

    // Print result
    console.log(JSON.stringify(image_racy_result, null, 2));
    console.log();
    ...
    let image_text_result = await client.imageModeration.oCRUrlInput('eng', 'application/json', imageBodyModel);
    console.log(JSON.stringify(image_text_result, null, 2));
    console.log();

    // Moderate faces content
    console.log(`Evaluating the image ${IMAGE_MODERATION_URLS[0].substring(IMAGE_MODERATION_URLS[0].lastIndexOf('/')+1)} for faces content...`)
    let image_faces_result = await client.imageModeration.findFacesUrlInput('application/json', imageBodyModel);
    console.log(JSON.stringify(image_faces_result, null, 2));
    /**
