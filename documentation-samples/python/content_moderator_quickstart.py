...
try:
    for label, urls in IMAGE_LIST.items():  # Get list of tuple pairs
        for url in urls:
            print('Adding image {} to list {} with label {}.'.format(
                url, image_list_id, label))
    ...
    # Prints content_id, additional_info, status, tracking_id
...
image_ids = client.list_management_image.get_all_image_ids(
    list_id=image_list_id)
assert isinstance(image_ids, ImageIds)
# Prints content_source, content_ids, status, tracking_id
...
updated_list = client.list_management_image_lists.update(
    list_id=image_list_id, content_type='application/json', body={'name': 'Swimsuits and sports'})
...
refresh_image_index = client.list_management_image_lists.refresh_index_method(
    list_id=image_list_id)
assert isinstance(refresh_image_index, RefreshIndex)
# Prints content_source_id, is_update_success, advanced_info, status, and tracking_id
pprint(refresh_image_index.as_dict())
...
for image_url in IMAGES_TO_MATCH:
    ...
    # Prints tracking_id, cache_id, is_match, matches, status
...
client.list_management_image.delete_image(
    list_id=image_list_id, image_id=index[image_to_remove])
...
client.list_management_term.add_term(
    list_id=terms_list_id, term="hello", language="eng")
client.list_management_term.add_term(
    list_id=terms_list_id, term="wave", language="eng")
...
terms = client.list_management_term.get_all_terms(
    list_id=terms_list_id, language="eng")
...
'''
refresh_terms_index = client.list_management_term_lists.refresh_index_method(
    list_id=terms_list_id, language="eng")
...
client.list_management_term.delete_term(
    list_id=terms_list_id, term=term_to_remove, language="eng")
...
'''
with open(os.path.join(TEXT_FOLDER, 'content_moderator_term_list.txt'), "rb") as text_fd:
    print('\nScreening text "{}" using term list {}'.format(
        terms_text, terms_list_id))
    screen = client.text_moderation.screen_text(text_content_type="text/plain", text_content=text_fd,
                                                language="eng", autocorrect=False, pii=False, list_id=terms_list_id)
    assert isinstance(screen, Screen)
    pprint(screen.as_dict())
